...
def get_project_root():
    ...
    for path in search_paths:
        try:
            if (path / "dbt").exists() and (path / "dagster_project").exists():
                return path.resolve()
    ...
    raise FileNotFoundError(
        ...
        "Required structure: must contain 'dbt/' and 'dagster_project/' subdirectories"
    )
...
DAGSTER_DIR = PROJECT_ROOT / "dagster_project"
